...
<html lang="en">
  ...
  <body>
    ...
    ?>
      <form action="actualizarLocalidad.php" method="post" style="width: 20rem;" class="bg-white p-3 rounded-2" id="loginForm">
      <h3>Actualizar localidad</h3>
            <input type="hidden" id="id" name="id" value="<?php echo $row['id_localidad']; ?>">
            <div class="mb-3">
                <label for="localidad" class="form-label">Localidad:</label>
                <input type="text" class="form-control" id="localidad" name="localidad" aria-describedby="emailHelp" value="<?php echo $row['nombre_localidad']; ?>">
              </div>
              <div class="mb-3">
                <label for="codigo_postal" class="form-label">Codigo postal:</label>
                <input type="text" class="form-control" id="codigo_postal" name="codigo_postal" aria-describedby="emailHelp" value="<?php echo $row['codigo_postal']; ?>">
              </div>
              <button type="submit" class="btn btn-primary">Editar</button>
              <a href="listaLocalidades.php" class="btn btn-danger">Volver atras</a>
        </form>
        <?php
      }else{
