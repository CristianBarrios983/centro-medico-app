...
<html lang="en">
  ...
  <body>
    ...
    <?php
      ...
      $query = "SELECT * FROM barrios WHERE id_barrio = $id_barrio";
    ...
    ?>
      <form action="actualizarBarrio.php" method="post" style="width: 20rem;" class="bg-white p-3 rounded-2" id="loginForm">
      <h3>Actualizar pais</h3>
            <input type="hidden" id="id" name="id" value="<?php echo $row['id_barrio']; ?>">
            <div class="mb-3">
                <label for="barrio" class="form-label">Barrio:</label>
                <input type="text" class="form-control" id="barrio" name="barrio" aria-describedby="emailHelp" value="<?php echo $row['nombre_barrio']; ?>">
              </div>
              <button type="submit" class="btn btn-primary">Editar</button>
              <a href="listaBarrios.php" class="btn btn-danger">Volver atras</a>
        </form>
        <?php
      }else{
